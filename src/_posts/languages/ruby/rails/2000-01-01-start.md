...
By default, this value is randomly generated by Scalingo and stored in the deployment cache, which may be deleted in some cases. You can change this default value by setting the `SECRET_KEY_BASE` environment variable in your application, and prevent issues in case the cache is deleted.
